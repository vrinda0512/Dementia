...
import { MemoryItemSystem } from "./MemoryItemSystem";
...
import { HidingPlaceId, InteractiveSceneObject, MemoryGamePhase, MemoryWorldCommand, WorldCallbacks } from "./types";

type GameWorldProps = WorldCallbacks & {
  audio: AudioManager;
  sessionActive: boolean;
  reducedMotion: boolean;
  memoryPhase: MemoryGamePhase;
  memoryCommand: MemoryWorldCommand;
};

type WorldMemoryController = {
  execute: (command: MemoryWorldCommand) => void;
};
...
export default function GameWorld({ audio, sessionActive, reducedMotion, memoryPhase, memoryCommand, onInteraction, onHoverChange, onWebGLFailure, onObservationComplete, onMemoryLocationSelect }: GameWorldProps) {
  const hostRef = useRef<HTMLDivElement>(null);
  const activeRef = useRef(sessionActive);
  const memoryPhaseRef = useRef(memoryPhase);
  const callbackRef = useRef({ onInteraction, onHoverChange, onObservationComplete, onMemoryLocationSelect });
  const memoryControllerRef = useRef<WorldMemoryController | null>(null);
  const memoryCommandRef = useRef(memoryCommand);
  ...
  useEffect(() => {
    memoryPhaseRef.current = memoryPhase;
  }, [memoryPhase]);

  useEffect(() => {
    callbackRef.current = { onInteraction, onHoverChange, onObservationComplete, onMemoryLocationSelect };
  }, [onInteraction, onHoverChange, onObservationComplete, onMemoryLocationSelect]);

  useEffect(() => {
    memoryCommandRef.current = memoryCommand;
    memoryControllerRef.current?.execute(memoryCommand);
  }, [memoryCommand]);
  ...
  useEffect(() => {
    ...
    const interactiveObjectById = new Map(interactiveObjects.map((sceneObject) => [sceneObject.id, sceneObject]));
    ...
    memoryControllerRef.current = {
      execute: (command) => {
        ...
        if (!command.itemId || !command.hidingPlaceId) return;
        const target = interactiveObjectById.get(command.hidingPlaceId);
        if (command.type === "observe") {
          memoryItems.observe(command.itemId, command.hidingPlaceId);
          return;
        }
        memoryItems.reveal(command.itemId, command.hidingPlaceId);
        target?.activate();
      },
    };
    memoryControllerRef.current.execute(memoryCommandRef.current);
    ...
    const activateInteraction = (event: MouseEvent | PointerEvent) => {
      ...
      if (memoryPhaseRef.current === "recalling") {
        if (response.objectId === "basket" || response.objectId === "kettle" || response.objectId === "photo") {
          callbackRef.current.onMemoryLocationSelect(response.objectId as HidingPlaceId);
        }
        return;
      }
      if (memoryPhaseRef.current !== "explore") return;
      ...
      callbackRef.current.onInteraction(response);
    };
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      if (pointerStart) {
        ...
        if (dragging) {
          ...
          callbackRef.current.onHoverChange(null);
          return;
        }
      }
      const hit = interactions.updatePointer(event, renderer.domElement, camera);
      callbackRef.current.onHoverChange(hit?.label ?? null);
    };
  ...
  }, [audio, onWebGLFailure, reducedMotion]);
